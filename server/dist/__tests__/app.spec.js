...
(0, vitest_1.describe)('Socket.io', () => {
    ...
    function connectRaw(boardId = 'main') {
        return new Promise((resolve, reject) => {
            const client = (0, socket_io_client_1.io)(`http://localhost:${port}`, {
                query: { boardId },
                transports: ['websocket'],
    ...
    // ── board:state on join ──────────────────────────────────────────────────
    (0, vitest_1.it)('emits board:state to the joining socket in response to user:join', async () => {
        const socket = await connectRaw();
        const statePromise = waitFor(socket, 'board:state');
        socket.emit('user:join');
        const state = await statePromise;
        (0, vitest_1.expect)(state).toEqual({ seq: 0, log: [] });
    });
    (0, vitest_1.it)('board:state includes existing log entries when joining mid-session', async () => {
        const first = await connect();
        await sendAction(first, basePayload);
        await sendAction(first, basePayload);
        const second = await connectRaw();
        const statePromise = waitFor(second, 'board:state');
        second.emit('user:join');
        const state = await statePromise;
        (0, vitest_1.expect)(state.log).toHaveLength(2);
        (0, vitest_1.expect)(state.seq).toBe(2);
    });
    (0, vitest_1.it)('does not send board:state to existing clients when a new socket joins', async () => {
        const first = await connect();
        let firstGotState = false;
        first.on('board:state', () => { firstGotState = true; });
        const second = await connectRaw();
        const secondStatePromise = waitFor(second, 'board:state');
        second.emit('user:join');
        await secondStatePromise;
        await new Promise((r) => setTimeout(r, 80));
        (0, vitest_1.expect)(firstGotState).toBe(false);
    });
    // ── presence events ──────────────────────────────────────────────────────
    (0, vitest_1.it)('emits user:joined to existing clients when a new socket connects', async () => {
        const first = await connect();
        const joinedPromise = waitFor(first, 'user:joined');
        const second = await connect();
        const joinedId = await joinedPromise;
        (0, vitest_1.expect)(joinedId).toBe(second.id);
    });
    (0, vitest_1.it)('emits user:left to remaining clients when a socket disconnects', async () => {
        const stayer = await connect();
        const leaver = await connect();
        const leftPromise = waitFor(stayer, 'user:left');
    ...
    (0, vitest_1.it)('broadcasts draw:action to all other connected clients', async () => {
        ...
        (0, vitest_1.expect)(received).toMatchObject(basePayload);
    });
    ...
    (0, vitest_1.it)('does not emit user:joined to clients on a different board', async () => {
        ...
        const sameBoardJoined = waitFor(existing, 'user:joined');
        await connect('board-a');
        await sameBoardJoined;
        await new Promise((r) => setTimeout(r, 80));
        (0, vitest_1.expect)(outsiderGotJoined).toBe(false);
    });
    (0, vitest_1.it)('does not emit user:left to clients on a different board', async () => {
        ...
        const sameBoardLeft = waitFor(stayer, 'user:left');
        leaver.disconnect();
        await sameBoardLeft;
        await new Promise((r) => setTimeout(r, 80));
    ...
    // ── draw:cursor relay ────────────────────────────────────────────────────
    (0, vitest_1.it)('broadcasts draw:cursor to other clients in the same room', async () => {
        const sender = await connect();
        const receiver = await connect();
        const receivedPromise = waitFor(receiver, 'draw:cursor');
        sender.emit('draw:cursor', { x: 42, y: 99 });
        const received = await receivedPromise;
        (0, vitest_1.expect)(received.x).toBe(42);
        (0, vitest_1.expect)(received.y).toBe(99);
    });
    (0, vitest_1.it)('stamps server-assigned socket.id as userId on draw:cursor', async () => {
        const sender = await connect();
        const receiver = await connect();
        const receivedPromise = waitFor(receiver, 'draw:cursor');
        sender.emit('draw:cursor', { x: 0, y: 0 });
        const received = await receivedPromise;
        (0, vitest_1.expect)(received.userId).toBe(sender.id);
    });
    (0, vitest_1.it)('does not echo draw:cursor back to the sender', async () => {
        const sender = await connect();
        const receiver = await connect();
        ...
        const receiverConfirmed = waitFor(receiver, 'draw:cursor');
        sender.emit('draw:cursor', { x: 1, y: 1 });
        await receiverConfirmed;
        await new Promise((r) => setTimeout(r, 80));
        (0, vitest_1.expect)(senderGotCursor).toBe(false);
    });
    (0, vitest_1.it)('does not deliver draw:cursor to clients on a different board', async () => {
        const sender = await connect('board-a');
        const sameBoard = await connect('board-a');
        const otherBoard = await connect('board-b');
        ...
        const sameBoardConfirmed = waitFor(sameBoard, 'draw:cursor');
        sender.emit('draw:cursor', { x: 5, y: 5 });
        await sameBoardConfirmed;
        await new Promise((r) => setTimeout(r, 80));
        (0, vitest_1.expect)(otherGotCursor).toBe(false);
    });
});
// ─── action log ──────────────────────────────────────────────────────────────
(0, vitest_1.describe)('draw:action log', () => {
    ...
    (0, vitest_1.it)('does not add draw:cursor events to the board log', async () => {
        const { boards, connectLog, close } = await makeLogServer();
        const sender = await connectLog();
        const receiver = await connectLog();
        const receivedPromise = waitFor(receiver, 'draw:cursor');
        sender.emit('draw:cursor', { x: 10, y: 20 });
        await receivedPromise;
        (0, vitest_1.expect)(boards.get('main')?.log ?? []).toHaveLength(0);
        await close();
    });
});
// ─── cursor state ─────────────────────────────────────────────────────────────
(0, vitest_1.describe)('cursor state', () => {
    const cursorClients = [];
    (0, vitest_1.afterEach)(() => cursorClients.forEach((c) => c.disconnect()));
    async function makeCursorServer() {
        const { httpServer, io, cursors } = (0, app_1.createApp)('*');
        await new Promise((r) => httpServer.listen(0, r));
        const port = httpServer.address().port;
        function connectCursor(boardId = 'main') {
            return new Promise((resolve, reject) => {
                const c = (0, socket_io_client_1.io)(`http://localhost:${port}`, {
                    query: { boardId },
                    transports: ['websocket'],
    ...
    (0, vitest_1.it)('stores cursor position when draw:cursor is received', async () => {
        ...
        (0, vitest_1.expect)(boardCursors.get(sender.id)).toMatchObject({ x: 42, y: 99, userId: sender.id });
        await close();
    });
    (0, vitest_1.it)('removes cursor entry when socket disconnects', async () => {
        const { cursors, connectCursor, close } = await makeCursorServer();
        const sender = await connectCursor();
        const receiver = await connectCursor();
        const receivedCursor = waitFor(receiver, 'draw:cursor');
        sender.emit('draw:cursor', { x: 1, y: 2 });
        await receivedCursor;
        const senderId = sender.id;
        (0, vitest_1.expect)(cursors.get('main')?.has(senderId)).toBe(true);
        const leftPromise = waitFor(receiver, 'user:left');
        sender.disconnect();
        await leftPromise;
        (0, vitest_1.expect)(cursors.get('main')?.has(senderId)).toBe(false);
        await close();
    });
    (0, vitest_1.it)('does not retain cursor entries for clients that never moved', async () => {
        const { cursors, connectCursor, close } = await makeCursorServer();
        const client = await connectCursor();
        const clientId = client.id;
        client.disconnect();
        await new Promise((r) => setTimeout(r, 80));
        (0, vitest_1.expect)(cursors.get('main')?.has(clientId)).toBeFalsy();
        await close();
    });
});
// ─── room cleanup ─────────────────────────────────────────────────────────────
(0, vitest_1.describe)('room cleanup', () => {
    const cleanupClients = [];
    (0, vitest_1.afterEach)(() => cleanupClients.forEach((c) => c.disconnect()));
    const IDLE_MS = 80;
    async function makeCleanupServer() {
        const { httpServer, io, boards, cursors, roomTimers } = (0, app_1.createApp)('*', { roomIdleMs: IDLE_MS });
        await new Promise((r) => httpServer.listen(0, r));
        const port = httpServer.address().port;
        function connectCleanup(boardId = 'main') {
            return new Promise((resolve, reject) => {
                const c = (0, socket_io_client_1.io)(`http://localhost:${port}`, {
                    query: { boardId },
                    transports: ['websocket'],
    ...
    (0, vitest_1.it)('deletes board log and cursors after idle timeout when room empties', async () => {
        const { boards, cursors, connectCleanup, close } = await makeCleanupServer();
        const client = await connectCleanup();
        await sendAction(client, basePayload);
        (0, vitest_1.expect)(boards.get('main')?.log).toHaveLength(1);
        client.disconnect();
        await new Promise((r) => setTimeout(r, IDLE_MS * 2));
        (0, vitest_1.expect)(boards.has('main')).toBe(false);
        (0, vitest_1.expect)(cursors.has('main')).toBe(false);
        await close();
    });
    (0, vitest_1.it)('cancels cleanup timer when a new client joins before timeout fires', async () => {
        ...
        await new Promise((r) => setTimeout(r, IDLE_MS / 2));
        (0, vitest_1.expect)(roomTimers.has('main')).toBe(true);
        const second = await connectCleanup();
        (0, vitest_1.expect)(roomTimers.has('main')).toBe(false);
        // Wait past where the original timer would have fired
        await new Promise((r) => setTimeout(r, IDLE_MS * 2));
        (0, vitest_1.expect)(boards.has('main')).toBe(true);
        second.disconnect();
        await close();
    });
    (0, vitest_1.it)('schedules a fresh timer when the room empties again after rejoining', async () => {
        const { boards, roomTimers, connectCleanup, close } = await makeCleanupServer();
        const first = await connectCleanup();
        first.disconnect();
        await new Promise((r) => setTimeout(r, IDLE_MS / 2));
    ...
    (0, vitest_1.it)('does not delete board while clients are still connected', async () => {
...
(0, vitest_1.describe)('Phase 3 — resilience', () => {
    const phase3Clients = [];
    (0, vitest_1.afterEach)(() => phase3Clients.forEach((c) => { try {
        c.disconnect();
    }
    catch { /* already gone */ } }));
    async function makeServer(opts = {}) {
        const { httpServer, io, boards } = (0, app_1.createApp)('*', opts);
        await new Promise((r) => httpServer.listen(0, r));
        const port = httpServer.address().port;
        function connectPhase3(boardId = 'main', extraOpts = {}) {
            return new Promise((resolve, reject) => {
                const c = (0, socket_io_client_1.io)(`http://localhost:${port}`, {
                    query: { boardId },
                    transports: ['websocket'],
    ...
    (0, vitest_1.it)('default 500-action cap is enforced when 600 events are sent', async () => {
        ...
        const board = boards.get('main');
        (0, vitest_1.expect)(board.log.length).toBe(500);
        (0, vitest_1.expect)(board.log[0].seq).toBe(101); // first 100 evicted
        (0, vitest_1.expect)(board.log[499].seq).toBe(600); // newest entry
        await close();
    });
    // ── scenario 4: abrupt disconnect → user:left ────────────────────────────
    (0, vitest_1.it)('broadcasts user:left when a connection is force-closed by the server', async () => {
        ...
        const leftPromise = waitFor(tab2, 'user:left', 1000);
        ...
        const leftId = await leftPromise;
        (0, vitest_1.expect)(leftId).toBe(tab1Id);
        (0, vitest_1.expect)(io.sockets.sockets.has(tab1Id)).toBe(false);
        await close();
    });
    // ── scenario 5: server shutdown → clients disconnect/reconnecting ─────────
    (0, vitest_1.it)('clients receive disconnect event when the server shuts down', async () => {
        const { io, connectPhase3, close } = await makeServer();
        const tab1 = await connectPhase3();
        const tab2 = await connectPhase3();
        const d1 = waitFor(tab1, 'disconnect', 3000);
        const d2 = waitFor(tab2, 'disconnect', 3000);
        // Shut down without waiting — clients should detect the dropped connection
        io.close();
        const [reason1, reason2] = await Promise.all([d1, d2]);
        (0, vitest_1.expect)(reason1).toMatch(/transport|server|close/i);
        (0, vitest_1.expect)(reason2).toMatch(/transport|server|close/i);
        await close();
    });
});
